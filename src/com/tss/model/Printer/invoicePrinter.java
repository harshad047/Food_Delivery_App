...
public class InvoicePrinter {
    ...
    public static void printInvoice(double discountedTotal, String paymentMode, String deliveryPartner, List<OrderItem> items, Order order) {
        ...
        System.out.printf("| %-6s | %-24s | %-5s | %-9s | %-12s |\n", "ID", "Item", "Qty", "Price", "Subtotal");
        System.out.println(border);
        ...
        for (OrderItem item : items) {
            ...
            System.out.printf("| %-6s | %-24s | %-5d | ₹%-8.2f | ₹%-11.2f |\n",
                    item.getId(), name, item.getQuantity(), item.getPrice(), item.getSubtotal());
        }

        System.out.println(border);
        System.out.printf("| %-45s : %-24s₹%-8.2f |\n", "Original Total", "", order.getTotal());
